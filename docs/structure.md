...
|-- eventloop.cpp       # epoll-based reactor, non-blocking client I/O, and shared string DB
...
- `eventloop.h` / `eventloop.cpp` own the single-threaded `epoll` loop, non-blocking server/client sockets, accept draining, per-client read/write readiness handling, parser feeding, response buffering, and the in-memory string database.
...
- `tests/test_v2_1.py` compiles the server and verifies epoll readiness behavior with idle clients and pipelined replies.
