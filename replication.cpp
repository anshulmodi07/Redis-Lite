...
{
    if (!full_sync_mode || g_databases == nullptr || master_read_buffer.size() < full_sync_expected)
    {
        return false;
    }
...
void processMasterBuffer()
{
    while (true)
    {
        ...
        if (master_read_buffer.rfind("+FULLRESYNC", 0) == 0)
        {
            ...
            full_sync_expected = static_cast<size_t>(stoull(line.substr(last_space + 1)));
            full_sync_mode = true;
            tryFinishFullSync();
            return;
        }
...
bool connectToMaster()
{
    ...
    if (inet_pton(AF_INET, g_server_config.replicaof_host.c_str(), &addr.sin_addr) <= 0)
    {
    ...
    if (connect(master_link_fd, reinterpret_cast<sockaddr*>(&addr), sizeof(addr)) < 0)
    {
        close(master_link_fd);
        master_link_fd = -1;
        return false;
    ...
    const string replconf = encodeArray({"REPLCONF", "listening-port", to_string(g_server_config.port)});
...
>>>>>>> 47531e2b4be68a3b819d30db016f0edfaedf772e
    {
...
void replicationSetContext(vector<RedisDb>* databases, unordered_map<int, Client>* clients, int epoll_fd)
{
...
{
    return fd == master_link_fd;
}

bool replicationHandleCommand(Client& client, const vector<string>& argv, string& reply)
{
    ...
    if (cmd == "PSYNC")
    {
        ...
        if (g_databases != nullptr)
        {
            ...
            reply = "+FULLRESYNC " + g_repl_id + " " + to_string(g_master_repl_offset) + " "
...
{
    if (g_server_config.readonly_replica || argv.empty() || g_clients == nullptr)
    {
        return;
    }
...
string replicationInfoSection()
{
    ...
    if (g_server_config.readonly_replica)
    {
        ...
        section += "master_port:" + to_string(g_server_config.replicaof_port) + "\r\n";
        section += "master_link_status:" + string(master_link_fd >= 0 ? "up" : "down") + "\r\n";
    }
